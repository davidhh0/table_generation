...
from llm_generation.prompts_generation import (
    ...
    prompt_generation_min_categorical,
    prompt_generation_min_numerical
)
import random
import os
...
def min_retrieval():
    ...
    for tbl in generated_tbl_cache.iterkeys():
        ...
        for col in comparable_columns:
            ...
            for cat_col in categorical_columns:
                ...
                answer_1 = answer_1[ answer_1[col] == answer_1[col].min()][key].tolist()
                answer_2 = df[df[cat_col] == third_common[0]]
                answer_2 = answer_2[ answer_2[col] == answer_2[col].min()][key].tolist()
                three_variants = prompt_generation_min_categorical(
                ...
                if not rephrase:
                    ...
                    tmp_json = cfg.copy()
                    previous_tasks = tmp_json.get("tasks", {}).get('min', {})
                    if 'count' not in tmp_json.get("tasks", {}):
                        tmp_json["tasks"] = {}
                        tmp_json["tasks"]['min'] = {}
                    three_variants['column'] = col
                    tmp_json["tasks"]['min'] = {**previous_tasks, **three_variants}
                ...
                for bd in three_variants:
                    ...
                    # CSV for model, task type, task, variant, num_rows, num_cols, popularity, numerical_cols_ratio, score
                        scores_statistics += f"{model},Min,Categorical,{bd},{_type},{df.shape[0]},{df.shape[1]},{tbl_statistics['popularity'][-1]},{tbl_statistics['numerical_cols_ratio'][-1]},{score}\n"
                break
            break
        if len(comparable_columns) > 1:
            ...
            if len(df[comparable_col_1].unique()) < 3 or len(df[comparable_col_2].unique()) < 3:
                continue
            three_variants = prompt_generation_min_numerical(
                table_desc=table_description,
    ...
    print(f"Popularity - Mean: {np.mean(tbl_statistics['popularity'])} - Std: {np.std(tbl_statistics['popularity'])}")
    print(f"Num rows - Mean: {np.mean(tbl_statistics['num_rows'])} - Std: {np.std(tbl_statistics['num_rows'])}")
    print(f"Num cols - Mean: {np.mean(tbl_statistics['num_cols'])} - Std: {np.std(tbl_statistics['num_cols'])}")
    print(f"Num cells - Mean: {np.mean(tbl_statistics['num_cells'])} - Std: {np.std(tbl_statistics['num_cells'])}")
    print(
        f"Numerical cols ratio - Mean: {np.mean(tbl_statistics['numerical_cols_ratio'])} - Std: {np.std(tbl_statistics['numerical_cols_ratio'])}")
    ...
    with open('scores.csv', 'a+') as f:
        ...
        for variant, details in categorical_score["categorical_equals"].items():
            f.write(f"{model},Min,Categorical,Equals,{variant},={details}/{categorical_tbls_processed}\n")
        for variant, details in categorical_score["categorical_not_equals"].items():
            f.write(f"{model},Min,Categorical,Not Equals,{variant},={details}/{categorical_tbls_processed}\n")
        for variant, details in categorical_score["categorical_in"].items():
            f.write(f"{model},Min,Categorical,In,{variant},={details}/{categorical_tbls_processed}\n")
        for variant, details in numerical_score["numerical_greater_than"].items():
            f.write(f"{model},Min,Numerical,Greater Than,{variant},={details}/{numerical_tbls_processed}\n")
        for variant, details in numerical_score["numerical_less_than"].items():
            f.write(f"{model},Min,Numerical,Less Than,{variant},={details}/{numerical_tbls_processed}\n")
        for variant, details in numerical_score["numerical_between"].items():
            f.write(f"{model},Min,Numerical,Between,{variant},={details}/{numerical_tbls_processed}\n")
        for variant, details in numerical_score["numerical_equals"].items():
            f.write(f"{model},Min,Numerical,Equals,{variant},={details}/{numerical_tbls_processed}\n")
