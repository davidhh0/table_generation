...
def count_retrieval():
    ...
    for tbl in generated_tbl_cache.iterkeys():
        ...
        for col in categorical_columns:
            if len(str(col)) == 1 or col.isnumeric():
                continue
            if len(Counter(df[col].tolist()).most_common(n=3)) < 3:
                continue
            ...
            three_variants = prompt_generation_count_categorical(
                table_desc=table_description,
                col=col,
                col_value_1=second_common[0],
                col_value_2=third_common[0],
                df=df,
                real_value_1=second_common[1],
                real_value_2=third_common[1],
            ...
            if not rephrase:
                ...
                if 'tasks' not in tmp_json:
                    tmp_json['tasks'] = {}
                tmp_json["tasks"]['count'] = {}
                if 'count' not in tmp_json.get("tasks", {}):
                    tmp_json["tasks"] = {}
                    tmp_json["tasks"]['count'] = {}
                three_variants['column'] = col
                tmp_json["tasks"]['count'] = {**previous_tasks, **three_variants}
            ...
            for bd in three_variants:
                    for _type, score in three_variants[bd].items():
                        ...
                        scores_statistics += f"{model},Count,Categorical,{bd},{_type},{df.shape[0]},{df.shape[1]},{tbl_statistics['popularity'][-1]},{tbl_statistics['numerical_cols_ratio'][-1]},{score}\n"
        ...
        for col in comparable_columns:
            ...
            if median_value == 0 or pd.isna(median_value):
                continue
            if (df[col] > median_value).sum() < 2 or (df[col] < median_value).sum() < 2:
                continue
            b = 5
            three_variants = prompt_generation_count_numerical(
                table_desc=table_description,
                col=col,
                df=df,
            ...
            if not rephrase:
                ...
                tmp_json = cfg.copy()
                previous_tasks = tmp_json.get("tasks", {}).get('count', {})
                if 'count' not in tmp_json.get("tasks", {}):
                    tmp_json["tasks"] = {}
                    tmp_json["tasks"]['count'] = {}
                tmp_json["tasks"]['count'] = {**previous_tasks, **three_variants}
    ...
    for record in single_value_scores.items():
        ...
        if "numerical" in data and list(data["numerical"].keys()):
            ...
            numerical_count += 1
            for variant, variant_details in data["numerical"][numerical_col].items():
                for _type, score in variant_details.items():
    ...
    print(
        f"Categorical in - Open book: {categorical_score['categorical_in']['open_book']} out of {categorical_count}"
    )
    # Numerical:
    #   Greater than
    print(
    ...
    #  Equals
    print(
        f"Numerical Equals - Closed book: {numerical_score['numerical_equals']['closed_book']} out of {numerical_count}"
    )
    print(
        f"Numerical Equals - Multiple choices: {numerical_score['numerical_equals']['multiple_choices']} out of {numerical_count}"
    )
    print(
        f"Numerical Equals - Open book: {numerical_score['numerical_equals']['open_book']} out of {numerical_count}"
    )
    print(f"Table statistics: ")
    print(f"Popularity - Min: {min(tbl_statistics['popularity'])}  Mean: {np.mean(tbl_statistics['popularity'])} - Max: {max(tbl_statistics['popularity'])} -"
          f" Std: {np.std(tbl_statistics['popularity'])}")
    print(f"Num rows - Min: {min(tbl_statistics['num_rows'])} Mean: {np.mean(tbl_statistics['num_rows'])} Max: {max(tbl_statistics['num_rows'])} - Std: {np.std(tbl_statistics['num_rows'])}")
    print(f"Num cols - Min: {min(tbl_statistics['num_cols'])}  Mean: {np.mean(tbl_statistics['num_cols'])} Max: {max(tbl_statistics['num_cols'])} - Std: {np.std(tbl_statistics['num_cols'])}")
    print(f"Num cells - Min: {min(tbl_statistics['num_cells'])} Mean: {np.mean(tbl_statistics['num_cells'])} Max: {max(tbl_statistics['num_cells'])} - Std: {np.std(tbl_statistics['num_cells'])}")
    print(f"Numerical cols ratio - Min: {min(tbl_statistics['numerical_cols_ratio'])} Mean: {np.mean(tbl_statistics['numerical_cols_ratio'])} Max: {max(tbl_statistics['numerical_cols_ratio'])} - Std: {np.std(tbl_statistics['numerical_cols_ratio'])}")
    print(f"Total tables with count task: {tbl_statistics['count']}")
    import utils.utils as _uu
    if _uu.COLLECT_MODE:
        return scores_statistics
    with open('scores.csv', 'a+') as f:
        # f.write("Model,TaskType,Task,Variant,Score\n")
        for variant, details in categorical_score["categorical_equals"].items():
            f.write(f"{model},Count,Categorical,Equals,{variant},={details}/{categorical_tbls_processed}\n")
        for variant, details in categorical_score["categorical_not_equals"].items():
            f.write(f"{model},Count,Categorical,Not Equals,{variant},={details}/{categorical_tbls_processed}\n")
        for variant, details in categorical_score["categorical_in"].items():
            f.write(f"{model},Count,Categorical,In,{variant},={details}/{categorical_tbls_processed}\n")
        for variant, details in numerical_score["numerical_greater_than"].items():
            f.write(f"{model},Count,Numerical,Greater Than,{variant},={details}/{numerical_tbls_processed}\n")
        for variant, details in numerical_score["numerical_less_than"].items():
            f.write(f"{model},Count,Numerical,Less Than,{variant},={details}/{numerical_tbls_processed}\n")
        for variant, details in numerical_score["numerical_between"].items():
            f.write(f"{model},Count,Numerical,Between,{variant},={details}/{numerical_tbls_processed}\n")
        for variant, details in numerical_score["numerical_equals"].items():
            f.write(f"{model},Count,Numerical,Equals,{variant},={details}/{numerical_tbls_processed}\n")
